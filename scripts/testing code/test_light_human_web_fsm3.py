...
def main():
    ...
    with sm:
        ...
        smach.StateMachine.add('Control_rosbridge', Control_rosbridge(), transitions={'next_state':'Control_human_detection','turn_off':'Turn_off'})

        smach.StateMachine.add('Control_human_detection', Control_human_detection(), transitions={'next_state':'monitor_for_human_detection_starting', 'turn_off':'Control_rosbridge'})

        smach.StateMachine.add('monitor_for_human_detection_starting', smach_ros.MonitorState("/sentry_control_topic", String, monitor_cb_human_detection_starting, output_keys=['data_received']), transitions={'invalid':'Pre_data_processing', 'valid':'monitor_for_human_detection_starting', 'preempted':'monitor_for_human_detection_starting'})

        smach.StateMachine.add('monitor_control', smach_ros.MonitorState("/sentry_control_topic", String, monitor_cb_control, output_keys=['control_data']), transitions={'invalid':'Control_human_detection', 'valid':'monitor_control', 'preempted':'monitor_control'}, remapping={'control_data':'control_data'})


        
        smach.StateMachine.add('Pre_data_processing', Pre_data_processing(), transitions={'turn_off':'Control_human_detection','monitor':'monitor_control', 'monitor_for_human':'monitor_for_human_detection_starting'}, remapping={'received_data':'data_received'})
